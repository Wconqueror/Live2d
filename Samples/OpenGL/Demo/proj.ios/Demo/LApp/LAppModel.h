...
public:
    ...
    /**
     * @brief model3.json从放置的目录和文件路径生成模型
     *
     */
     ...
     *
     * @param[in]   hitAreaName     要进行命中判断的目标ID
     * @param[in]   x               判定を行うX座標
     * @param[in]   y               判定を行うY座標
     */
...
private:
    ...
    Csm::csmVector<Csm::CubismIdHandle> _lipSyncIds; ///< 唇形同步功能的参数在模型中设置ID
    Csm::csmMap<Csm::csmString, Csm::ACubismMotion*>   _motions; ///< 加载动作列表
